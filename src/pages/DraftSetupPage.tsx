...
import type { GameCreationInfo } from '../types';
...
export function DraftSetupPage({ onEnterDraft }: DraftSetupPageProps) {
  ...
  async function handleManifestRitual() {
    ...
    setCreateError(null);
    try {
      // Frontend generates playerID as a UUID before calling POST /game.
      const payload: GameCreationInfo = {
        gameID, // backend assigns this on creation — confirm this assumption
        cubeID: cubeID.trim(),
        numberOfDoubleDraftPicksPerPlayer: extraPicks,
        players: [
          { playerID: crypto.randomUUID(), name: yourName.trim() },
          { playerID: crypto.randomUUID(), name: partnerTag.trim() },
        ],
      };
        ...
        <div className="max-w-[1200px] mx-auto relative z-10 px-4">
          <section className="text-center mb-xl">
            ...
            <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto">
              Two person draft of a cube. Hate drafting is more difficult. Easier to have a plan going in to a draft. Will see over 90% of a standard cube size. See the <a href="www.google.com">wordpress</a> here for more information.
            </p>
          </section>
