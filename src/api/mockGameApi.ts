import type { Card, CardDetail, CardPack, GameCreationInfo, GameInfo, GameStatusMessage, Player } from '../types';
...
function buildPlayer(name: string, playerID: string, packCount: number, doublePicks: number): Player {
...
export const mockGameApi = {
  async createAndStartGame(creationInfo: GameCreationInfo): Promise<GameInfo> {
    ...
    const players = creationInfo.players.map((p) =>
      buildPlayer(p.name || 'Player', p.playerID, packCount, creationInfo.numberOfDoubleDraftPicksPerPlayer)
    );
  ...
  async draftCard(
    ...
    if (!player) throw new Error(`Mock player ${playerID} not found`);
